...
pop_offline_messages(Acc, User, Server) ->
    mongoose_acc:append(offline, messages, offline_chatmarkers(User, Server), Acc).
...
offline_chatmarkers(User, Server) ->
    JID = jid:make(User, Server, <<"">>),
    {ok, Rows} = mod_offline_chatmarkers_backend:get(JID),
    mod_offline_chatmarkers_backend:remove_user(JID),
    lists:concat([process_row(JID, R) || R <- Rows]).

process_row(Jid, {Thread, undefined, TS}) ->
    ChatMarkers = mod_smart_markers:get_chat_markers(one2one, Jid, Thread, TS),
    [build_one2one_chatmarker_msg(CM) || CM <- ChatMarkers];
process_row(Jid, {Thread, Room, TS}) ->
    ChatMarkers = mod_smart_markers:get_chat_markers(groupchat, Room, Thread, TS),
    [build_room_chatmarker_msg(Jid, CM) || CM <- ChatMarkers].

build_one2one_chatmarker_msg(CM) ->
    #{from := From, to := To, thread := Thread, type := Type, id := Id} = CM,
    Children = thread(Thread) ++ marker(Type, Id),
    Attributes = [{<<"from">>, jid:to_binary(From)},
                  {<<"to">>, jid:to_binary(To)}],
    #xmlel{name = <<"message">>, attrs = Attributes, children = Children}.

build_room_chatmarker_msg(To, CM) ->
    #{from := From, to := Room, thread := Thread, type := Type, id := Id} = CM,
    FromBin = jid:to_binary(Room#jid.luser, Room#jid.lserver, jid:to_binary(jid:to_lus(From))),
    Children = thread(Thread) ++ marker(Type, Id),
