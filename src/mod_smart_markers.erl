...
-callback get_chat_markers(Host :: jid:lserver(), To :: jid:simple_bare_jid(),
...
get_chat_markers(ChatType, #jid{lserver = LServer} = To, Thread, TS) ->
    ...
    Host = case ChatType of
               one2one -> LServer;
               groupchat -> mongoose_subhosts:get_host(LServer)
           end,
...
-spec extract_chat_markers(mongoose_acc:t(), From :: jid:jid(), To :: jid:jid(),
...
get_chat_markers(#xmlel{children = Children}) ->
    lists:filter(fun is_chat_marker_element/1, Children).
